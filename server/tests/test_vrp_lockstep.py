"""Drift guard: the installed ``sm_arp.vrp`` (the published VRP library this chapter
now depends on) MUST stay byte-for-byte equivalent to the canonical ``conformance/vrp``.

The chapter un-vendored its VRP core — ``chapter/vrp.py`` imports ``sm_arp.vrp`` instead
of a vendored copy. ``conformance/vrp`` remains the source of truth: if the published
library drifts, this chapter would compute a different ``behavioral_merkle_root`` /
``nanda-rep`` score / attestation than another chapter or an external resolver, defeating
the "everyone recomputes the same value" property. This guard runs both implementations
over fixed inputs and asserts identical output. Skips standalone (no ``conformance/``
``conformance/`` alongside).
"""
