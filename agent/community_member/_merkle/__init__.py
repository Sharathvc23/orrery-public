...
VENDORED copy for the member SDK. Byte-for-byte identical (except this docstring)
to the canonical ``conformance/merkle/__init__.py``; the member ships as a pip
package and cannot import ``conformance/`` at runtime. The drift guard
``tests/test_merkle_lockstep.py`` enforces lockstep. Do not edit Merkle semantics
here — change ``conformance/merkle`` and re-vendor.
...
binary tree, odd nodes duplicated) — different commitments, never interchanged.
