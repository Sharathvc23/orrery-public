...
VENDORED copy for the member SDK. Byte-for-byte identical (except this
docstring) to the canonical ``conformance/dat/__init__.py``; the member ships
as a pip package and cannot import ``conformance/`` at runtime. The drift guard
``tests/test_dat_lockstep.py`` enforces lockstep with the canonical source.
Do not edit DAT semantics here — change ``conformance/dat`` and re-vendor.
