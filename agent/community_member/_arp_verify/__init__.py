"""ARP v0.1 conformance verifier — vendored copy for the member SDK.

VERBATIM COPY of ``conformance/arp/__init__.py`` (from the repo root)
with one change: ``SCHEMA_ROOT`` points at the local ``./schemas/`` folder
(which holds per-version subdirs ``0.1/`` and ``0.2/``), shipped inside the
``community-member`` wheel as package data.

Why vendoring instead of dynamic import: ``community-member`` is a pip
package; the repo's ``conformance/`` directory does not ship with it.
Importing ``conformance.arp`` at runtime would crash any pip-installed SDK.
The member SDK is intentionally self-contained;
cross-package imports from ``conformance/`` would violate that boundary.

This mirrors the chapter's ``chapter/_arp_verify/`` vendoring, which exists
for the same reason (the chapter Docker context excludes ``conformance/``).

Drift discipline: this file MUST stay byte-for-byte in lockstep with
``conformance/arp/__init__.py`` except for the SCHEMA_ROOT line. The test
``tests/test_arp_verify_lockstep.py`` enforces this (verifier code AND the
per-version schemas) — any change to one must land in the other, or CI fails.
...
The verifier is intentionally framework-agnostic: nothing in this module
imports org-runtime or member-sdk modules. Any ARP implementation can
run vectors against its own emit pipeline by importing ``verify_receipt``.
"""
