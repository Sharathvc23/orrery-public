...
CANONICAL source for DAT verification. Runtimes consume this verbatim:

* the chapter keeps an in-tree copy at ``chapter/dat.py`` (its Docker context
  already includes it);
* the member SDK vendors it at ``community_member/_dat/`` (it ships as a pip
  package and cannot import ``conformance/`` at runtime).

Both copies are held byte-for-byte in lockstep with this file by drift-guard
tests; the only permitted difference is the module docstring. Any change to DAT
verification semantics lands HERE first, then propagates — exactly the
spec-canonical / no-drift discipline this repo enforces.

ARP records *what an agent did*; a DAT records *what it was allowed to do*. The
key distinction from an ARP receipt: a DAT is signed by the **grantor** (the
principal, or a delegate already authorized to grant), not by the acting agent —
so authority is traceable, never self-claimed.
