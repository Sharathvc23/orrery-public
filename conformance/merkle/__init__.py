...
CANONICAL source for RFC 6962 Merkle verification. Runtimes consume this
verbatim: the chapter keeps an in-tree copy at ``chapter/merkle.py`` (its Docker
context already includes it) and the member SDK vendors it at
``community_member/_merkle/`` (it ships as a pip package). All three are held
byte-for-byte in lockstep by drift-guard tests; only the docstring may differ.
Change Merkle semantics HERE first, then propagate.

NOTE: this is the *inclusion/consistency-proof* tree (domain-separated 0x00/0x01
hashes, odd nodes promoted). It is NOT the VRP ``behavioral_merkle_root`` (plain
binary tree, odd nodes duplicated) — those are different commitments and must
never be interchanged. See the design doc §4.
