"""ARP conformance harness — version-aware (arp/0.1, arp/0.2).

The ``version`` field on each receipt selects its schema set
(``schema/arp/0.1`` or ``schema/arp/0.2``); an unknown version is a schema
failure. v0.1 receipts remain valid (mandatory backward-compat window).
...
SCHEMA_ROOT = Path(__file__).resolve().parent.parent.parent / "schema" / "arp"
